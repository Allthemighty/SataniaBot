...
class Functional_Commands():
    ...
    async def summarize(self, ctx, link, length=7):
        ...
        smkey = os.getenv('SMKEY')
        host = 'https://api.smmry.com'
        path = '/&SM_API_KEY={}&SM_URL='.format(smkey)
        if int(length) > 15:
            length = 15
        sumlen = '&SM_LENGTH=' + '{}'.format(length)
        superlink = host + path + link + sumlen
        resp = requests.get(url=superlink)
        data = resp.json()
        print(data)
        if 'sm_api_error' in data:
            errorcode = data['sm_api_error']
            if errorcode is 0:
...
def setup(bot):
    bot.add_cog(Functional_Commands(bot))
